...
export function ReviewFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (isEdit) {
      await reviewsHttp.save({ id: Number(id), ...form } as any);
    } else {
      await reviewsHttp.create(form as any);
    }
  ...
  return (
    <div>
      <div className="form-header">
        <h1 className="title-text">{isEdit ? 'Редактировать отзыв' : 'Добавить отзыв'}</h1>
        <button type="button" className="btn btn-secondary" onClick={() => navigate('/dashboard/reviews')}>Назад</button>
      </div>
